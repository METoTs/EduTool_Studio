...
현재 집계: {"실패":9,"통과":21,"부분 확인":27,"기존 통과":4,"차단":36} / 총 97건.
...
| TC-107 | 기능별 녹화 설정을 기본값으로 초기화 | 부분 확인 | 녹화 기본 설정 초기화 버튼으로 저장 경로가 사용자 Videos 기본값으로 변경됨. 취소해 기존 경로 유지. 인증된 계정·타 기능 설정의 보존은 미확인. | [TC-107-record-settings.png](../tc-screenshots/full-tc-20260923/TC-107-record-settings.png)<br>[TC-107-reset.png](../tc-screenshots/full-tc-20260923/TC-107-reset.png) |
| TC-108 | 쓰기 불가 폴더를 저장 위치로 지정하고 녹화 시작 | 차단 | 쓰기 권한이 제거된 전용 시험 폴더가 없음. 기존 잘못된 경로 오류 확인은 권한 거부 시험을 대체하지 않음. |  |
...
| TC-112 | 신규 카메라를 PIP 위치 또는 전체 화면으로 배치 | 부분 확인 | PIP 및 전체 화면 배치 프리뷰 확인. 녹화 파일은 프로세스 종료 후 183.367초 파일로 확인됐으나 배치별 결과 프레임 대조는 미완료. | [TC-112-camera-added.png](../tc-screenshots/full-tc-20260923/TC-112-camera-added.png)<br>[TC-112-fullscreen-layout.png](../tc-screenshots/full-tc-20260923/TC-112-fullscreen-layout.png)<br>[shutdown-record-probe.json](../tc-screenshots/full-tc-20260923/shutdown-record-probe.json) |
...
| TC-115 | 카메라 해상도/FPS 변경 후 적용 | 부분 확인 | UI 해상도 요약1280×720 변경 및 재시작 후 카메라1280×720 초기화 확인. 변경 직후 로그는640×480만 보여 실시간 반영을 확정하지 못함. 이후 종료 정지는 TC-127에 별도 실패 기록하며 해상도 변경과 인과관계는 미확정. | [TC-115-applied.png](../tc-screenshots/full-tc-20260923/TC-115-applied.png)<br>[restart-after-hang.png](../tc-screenshots/full-tc-20260923/restart-after-hang.png) |
...
| TC-123 | 지연 녹화 없이 시작 | 통과 | 지연 0에서 즉시 REC 시간 증가·실제 1280x720 H264/AAC 녹화 파일 기록 확인. 파일 크기는 프로세스 종료 후 디스크 반영됨. 종료 실패는 TC-127 별도. | [TC-123-immediate-camera.png](../tc-screenshots/full-tc-20260923/TC-123-immediate-camera.png)<br>[shutdown-record-probe.json](../tc-screenshots/full-tc-20260923/shutdown-record-probe.json) |
...
| TC-125 | 녹화 중 시작 버튼 반복 클릭 | 부분 확인 | 녹화 시작 후 버튼이 녹화 중단으로 바뀌어 동일 시작 동작을 반복할 수 없음. 지연 모달 중 중복 입력 및 별도 단축키 경합은 미확인. | [TC-124-countdown.png](../tc-screenshots/full-tc-20260923/TC-124-countdown.png)<br>[TC-123-immediate-camera.png](../tc-screenshots/full-tc-20260923/TC-123-immediate-camera.png) |
...
| TC-127 | 녹화 중 앱 종료 요청 | 실패 | 종료 확인 후 Shutting down 단계에서 응답 정지(REC 26초 화면 고정). 수 분 후 시험 프로세스 강제 종료 필요. 실행 중 파일 크기는 0바이트로 관찰됐으나 종료 후 137,637,928바이트/183.367초 H264·AAC 파일이 남음. 즉 파일 없음/손상으로 단정하지 않으며 정상 종료·녹화 정지 실패로 판정. | [TC-127-cancel.png](../tc-screenshots/full-tc-20260923/TC-127-cancel.png)<br>[TC-127-shutdown-hang.png](../tc-screenshots/full-tc-20260923/TC-127-shutdown-hang.png)<br>[shutdown-record-probe.json](../tc-screenshots/full-tc-20260923/shutdown-record-probe.json)<br>[recording-session.log](../tc-screenshots/full-tc-20260923/recording-session.log) |
...
| TC-501 | 정상 MP4를 파일 선택으로 열기 | 부분 확인 | 8초 H264/AAC 320x240 24fps 파일 열기·타임라인·재생 확인. 우측 FPS 문자열이 잘려 전체 정보 표시 기대는 충족하지 못함. | [TC-501-import.png](../tc-screenshots/full-tc-20260923/TC-501-import.png)<br>[TC-506-play.png](../tc-screenshots/full-tc-20260923/TC-506-play.png) |
| TC-502 | 정상 영상을 창에 끌어놓기 | 차단 | 탐색기에서 실제 드래그를 시도했으나 Computer Use가 창 경계 밖 드롭 좌표를 거부하여 앱 동작은 미검증. |  |
| TC-503 | 최근 파일에서 영상 열기 | 실패 | 이동한 원본을 최근 목록에서 선택하면 영상 또는 ffprobe를 찾을 수 없다는 일반 오류만 표시. 누락 항목 정리/위치 찾기 안내가 없고 목록에 그대로 남음. 기존 편집은 유지. | [TC-503-error.png](../tc-screenshots/full-tc-20260923/TC-503-error.png) |
| TC-504 | 손상된 파일 또는 지원하지 않는 코덱 열기 | 통과 | 손상된 invalid.mp4 추가 시 파일 경로를 포함한 읽기 오류 표시. 기존 8초 클립/타임라인 유지. | [TC-504-invalid-preserved.png](../tc-screenshots/full-tc-20260923/TC-504-invalid-preserved.png) |
| TC-505 | 두 번째 영상을 추가하거나 새로 불러오기 | 통과 | 영상 추가는 기존3조각에 AVI를 앞에 넣어4조각/11초 유지. 새 영상 열기는 미저장 저장/저장안함/취소 확인을 거쳐 저장안함 선택 시 단일 mixed-export.mp4 작업으로 교체. | [TC-520-front.png](../tc-screenshots/full-tc-20260923/TC-520-front.png)<br>[TC-505-confirm.png](../tc-screenshots/full-tc-20260923/TC-505-confirm.png)<br>[TC-505-new-loaded.png](../tc-screenshots/full-tc-20260923/TC-505-new-loaded.png) |
| TC-506 | 재생·일시정지·임의 시점 탐색 | 부분 확인 | 합성 영상 재생 시 화면 타임코드와 현재 시간/플레이헤드 증가 확인. 실음성 청감 동기화는 미확인. | [TC-507-fullscreen.png](../tc-screenshots/full-tc-20260923/TC-507-fullscreen.png) |
| TC-507 | 볼륨 변경·전체화면 진입/종료 | 실패 | 전체화면 진입 후 Escape 복귀 시 영상 위젯이 편집 영역으로 돌아오지 않고 별도 obs64 창으로 남음. 해당 창 닫으면 편집 미리보기 자체가 사라짐. | [TC-507-fullscreen.png](../tc-screenshots/full-tc-20260923/TC-507-fullscreen.png)<br>[TC-507-focus-escape.png](../tc-screenshots/full-tc-20260923/TC-507-focus-escape.png)<br>[TC-507-close-floating.png](../tc-screenshots/full-tc-20260923/TC-507-close-floating.png) |
| TC-508 | 타임라인 확대/축소 | 통과 | 선택 0.980~2.957초·전체 8초·위치 0.980초 유지하며 확대 배율 1→5 변경. 전체 보기에서 1로 축소 확인. | [TC-511-selection.png](../tc-screenshots/full-tc-20260923/TC-511-selection.png)<br>[TC-508-zoom.png](../tc-screenshots/full-tc-20260923/TC-508-zoom.png)<br>[TC-509-fit.png](../tc-screenshots/full-tc-20260923/TC-509-fit.png) |
| TC-509 | `전체 보기` 선택 | 통과 | 전체 보기가 8초 전체를 영역에 맞추고 재생 위치 0.980초 유지. | [TC-509-fit.png](../tc-screenshots/full-tc-20260923/TC-509-fit.png) |
| TC-510 | 긴 영상에서 타임라인 스크롤·탐색 | 통과 | 10분·1fps 시험 영상에서 5배 확대, 가로 스크롤, 307.161초 탐색 수행. 영상 내 307초 표기와 1fps 오차 범위 내 일치, 멈춤 없음. 고해상도 부하 시험은 별도. | [TC-510-zoom.png](../tc-screenshots/full-tc-20260923/TC-510-zoom.png)<br>[TC-510-scroll.png](../tc-screenshots/full-tc-20260923/TC-510-scroll.png)<br>[TC-510-seek.png](../tc-screenshots/full-tc-20260923/TC-510-seek.png) |
| TC-511 | 마우스 드래그로 구간 선택 | 부분 확인 | 드래그 선택 0.980~2.957초와 강조 일치. 구간 길이 1.977초의 독립 표시가 없어 기대 항목 전체 충족은 아님. | [TC-511-selection.png](../tc-screenshots/full-tc-20260923/TC-511-selection.png) |
...
| TC-516 | 영상 중간의 재생 위치에서 나누기 | 통과 | 8초 영상을 3.979초에서 분할하여 0~3.979 / 3.979~8.000의 합 8초 유지. | [TC-516-split.png](../tc-screenshots/full-tc-20260923/TC-516-split.png) |
| TC-517 | 여러 번 나누고 조각 이름 수정 | 통과 | 첫 조각 이름을 TC 첫 번째 조각으로 변경 후 두 번째 조각을 5.993초에서 추가 분할. 세 조각 이름·순서·경계·전체 8초 유지. | [TC-517-renamed.png](../tc-screenshots/full-tc-20260923/TC-517-renamed.png)<br>[TC-517-multiple.png](../tc-screenshots/full-tc-20260923/TC-517-multiple.png)<br>[TC-524-named-saved.png](../tc-screenshots/full-tc-20260923/TC-524-named-saved.png) |
| TC-518 | 조각 하나를 저장 | 부분 확인 | 3.979~5.993초 조각만 single-clip.mp4로 저장. ffprobe: H264 1280×720 30fps 영상2.000초/AAC2.035333초. 실제 재생 음향 확인은 미완료. | [TC-518-export.png](../tc-screenshots/full-tc-20260923/TC-518-export.png) |
...
| TC-520 | 복수 영상을 추가해 앞/현재 위치/뒤에 각각 삽입 | 부분 확인 | 3초 AVI 맨 앞 삽입으로 8→11초 및 앞 순서 확인. 이전 뒤 추가는 73.4초 확인. 현재 위치 삽입은 미검증이며 경계 재생 실패(TC-521). | [TC-520-front.png](../tc-screenshots/full-tc-20260923/TC-520-front.png)<br>[TC-521-stalled.png](../tc-screenshots/full-tc-20260923/TC-521-stalled.png) |
| TC-521 | 합친 영상의 경계 전후 재생 | 실패 | 3초 AVI 뒤에 분할 MP4를 합쳐 2.475초부터 재생. 6.979초(분할 조각 경계)에서 검은 미리보기와 시간이 멈추고 재관찰에도 동일. | [TC-521-before-boundary.png](../tc-screenshots/full-tc-20260923/TC-521-before-boundary.png)<br>[TC-521-play.png](../tc-screenshots/full-tc-20260923/TC-521-play.png)<br>[TC-521-boundary-result.png](../tc-screenshots/full-tc-20260923/TC-521-boundary-result.png)<br>[TC-521-stalled.png](../tc-screenshots/full-tc-20260923/TC-521-stalled.png) |
| TC-522 | 서로 다른 해상도/FPS/코덱의 영상 합치기 | 통과 | 320×240 24fps H264/AAC MP4와640×360 25fps MPEG4 무음 AVI를 합쳐 앱에서 내보냄. 결과H2641280×720 30fps/AAC11.021333초, 전체 ffmpeg 디코딩exit0. 변환 정책 일치. 편집 중 경계 재생 결함은TC-521 별도. | [TC-520-front.png](../tc-screenshots/full-tc-20260923/TC-520-front.png)<br>[TC-522-complete.png](../tc-screenshots/full-tc-20260923/TC-522-complete.png)<br>[mixed-probe.json](../tc-screenshots/full-tc-20260923/mixed-probe.json) |
| TC-523 | 합치기 실행 취소/다시 실행 | 실패 | 앞에 3초 AVI 삽입 전 위치5.993초. 실행 취소로 3클립/8초/순서는 복원되나 위치가0초로 바뀜. 다시 실행은4클립/11초 복원하나 정지 미리보기가 검게 표시됨. | [TC-520-front.png](../tc-screenshots/full-tc-20260923/TC-520-front.png)<br>[TC-523-undo.png](../tc-screenshots/full-tc-20260923/TC-523-undo.png)<br>[TC-523-redo.png](../tc-screenshots/full-tc-20260923/TC-523-redo.png) |
| TC-524 | 편집 작업 저장 후 앱 재시작·다시 열기 | 통과 | 앱 재시작 후 저장 작업의 3조각 이름/구간/순서/8초 길이와 선택 0.980~2.957초, 위치 5.993초 복원 확인. 이전 병합 작업도 복원. | [TC-524-named-saved.png](../tc-screenshots/full-tc-20260923/TC-524-named-saved.png)<br>[TC-525-fit.png](../tc-screenshots/full-tc-20260923/TC-525-fit.png)<br>[TC-524-restored.png](../tc-screenshots/full-tc-20260923/TC-524-restored.png) |
| TC-525 | 원본 파일 이동/삭제 후 저장 작업 열기 | 통과 | 시험용 원본 이름 변경 후 저장 작업 열기에서 누락 원본 재연결 창 표시. 각 3조각을 이동 파일에 연결하여 이름·경계·총 8초·선택 0.980~2.957초·위치 5.993초 및 영상 복원. | [TC-525-missing.png](../tc-screenshots/full-tc-20260923/TC-525-missing.png)<br>[TC-525-fit.png](../tc-screenshots/full-tc-20260923/TC-525-fit.png) |
...
| TC-527 | 내보내기 경로 쓰기 불가/파일명 충돌 | 부분 확인 | 기존 single-clip.mp4와 같은 이름으로 내보내기 시 덮어쓰기 확인 창 표시. 아니요 선택으로 경로 선택에 복귀하고 취소함. 쓰기 권한 없는 경로 분기는 미검증. | [TC-527-overwrite-prompt.png](../tc-screenshots/full-tc-20260923/TC-527-overwrite-prompt.png)<br>[TC-527-declined.png](../tc-screenshots/full-tc-20260923/TC-527-declined.png) |
| TC-528 | 내보내기 중 취소/오류 | 부분 확인 | 10분 영상 내보내기 중 작업 취소: 취소 안내 및 600초 편집·307.161초 위치 유지. 최종 cancel-export.mp4가 남지 않음. 강제 인코더 오류 분기는 미검증. | [TC-528-exporting.png](../tc-screenshots/full-tc-20260923/TC-528-exporting.png)<br>[TC-528-cancelled.png](../tc-screenshots/full-tc-20260923/TC-528-cancelled.png) |
